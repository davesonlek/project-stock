...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
        <h6 class="mb-0 fw-bold"><i class="bi bi-rulers me-2 text-primary"></i>Units of Measure</h6>
        @can('create', \Modules\MasterData\Models\Unit::class)
    ...
    <div class="card-body border-bottom bg-light py-2">
        <form method="GET" action="{{ route('units.index') }}" class="row g-2 align-items-center">
            <div class="col-md-4">
            ...
            <div class="col-md-2">
                <a href="{{ route('units.index') }}" class="btn btn-sm btn-outline-secondary w-100">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($units as $unit)
                        <tr>
                            <td class="fw-semibold text-primary">{{ $unit->code }}</td>
                            <td>{{ $unit->name }}</td>
                            <td>
                                @if($unit->is_active)
                            ...
                            <td class="text-end">
                                @can('update', $unit)
                                    <a href="{{ route('units.edit', $unit->id) }}" class="btn btn-sm btn-outline-primary py-0 px-2 me-1">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    @if($unit->is_active)
                                        <form method="POST" action="{{ route('units.deactivate', $unit->id) }}" class="d-inline">
                                            @csrf
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('units.activate', $unit->id) }}" class="d-inline">
                                            @csrf
